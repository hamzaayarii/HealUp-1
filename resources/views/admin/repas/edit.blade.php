...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h1 class="h3 mb-0">✏️ Edit Meal</h1>
            <p class="text-muted">Update meal information</p>
        </div>
        <a href="{{ route('admin.nutrition.repas.show', $repas) }}" class="btn btn-outline-secondary">
    ...
    <form action="{{ route('admin.nutrition.repas.update', $repas) }}" method="POST" id="repasForm">
        @csrf
        @method('PUT')
                        ...
                        <div class="row mb-3">
                            <div class="col-md-6">
                                ...
                                <select class="form-select" id="user_id" name="user_id" required>
                                    @foreach($users as $user)
                                        <option value="{{ $user->id }}" {{ old('user_id', $repas->user_id) == $user->id ? 'selected' : '' }}>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control" id="nom" name="nom" 
                                       value="{{ old('nom', $repas->nom) }}" required>
                            </div>
                        </div>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="datetime-local" class="form-control" id="date_consommation" 
                                       name="date_consommation" value="{{ old('date_consommation', $repas->date_consommation->format('Y-m-d\TH:i')) }}" required>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="admin-card mb-4">
                    ...
                    <div class="card-body">
                        <div id="ingredients-container">
                            @foreach($repas->repasIngredients as $index => $ri)
                            <div class="ingredient-item border rounded p-3 mb-3 bg-light">
                                        ...
                                        <select name="ingredients[{{ $index }}][id]" class="form-select ingredient-select" required>
                                            @foreach($ingredients as $ingredient)
                                                <option value="{{ $ingredient->id }}" 
                                                        {{ $ri->ingredient_id == $ingredient->id ? 'selected' : '' }}
                                ...
                                <div class="mt-2 p-2 bg-white rounded border ingredient-preview"></div>
                            </div>
                            @endforeach
                        </div>
                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    let ingredientIndex = {{ count($repas->repasIngredients) }};
    ...
    document.getElementById('add-ingredient').addEventListener('click', function() {
        ...
        const newHtml = ingredientTemplate.replace(/INDEX/g, ingredientIndex);
    ...
    // Attach listeners to existing ingredients
    document.querySelectorAll('.ingredient-item').forEach(item => {
        attachIngredientListeners(item);
    });
