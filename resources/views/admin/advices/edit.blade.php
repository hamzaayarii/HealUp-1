<div class="p-6 space-y-4">
    <form action="{{ route('admin.advices.update', $advice) }}" method="POST" class="space-y-4">
        ...
        <div>
            ...
            >{{ old('content', $advice->content) }}</textarea>
            @error('content')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            >
                @foreach(\App\Models\User::all() as $user)
                    <option value="{{ $user->id }}" {{ old('user_id', $advice->user_id) == $user->id ? 'selected' : '' }}>
        ...
        <div>
            ...
            >
                <option value="AI" {{ old('source', $advice->source) == 'AI' ? 'selected' : '' }}>AI</option>
                <option value="professor" {{ old('source', $advice->source) == 'professor' ? 'selected' : '' }}>Professor</option>
                <option value="system" {{ old('source', $advice->source) == 'system' ? 'selected' : '' }}>System</option>
            </select>
            @error('source')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        ...
        <div>
            ...
            <select name="is_read" class="w-full border rounded px-3 py-2">
                <option value="0" {{ old('is_read', $advice->is_read) == 0 ? 'selected' : '' }}>Unread</option>
                <option value="1" {{ old('is_read', $advice->is_read) == 1 ? 'selected' : '' }}>Read</option>
            </select>
        </div>
