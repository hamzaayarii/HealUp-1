...
<div class="container mx-auto py-8">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-3xl font-bold text-blue-700">✏️ Modifier Repas</h1>
        <div class="flex space-x-2">
            <a href="{{ route('repas.index') }}" class="bg-gray-500 hover:bg-gray-600 text-white font-semibold py-2 px-4 rounded">Retour</a>
            <a href="{{ route('repas.show', $repas) }}" class="bg-blue-500 hover:bg-blue-600 text-white font-semibold py-2 px-4 rounded">Voir</a>
        </div>
    </div>
        ...
        <form action="{{ route('repas.update', $repas) }}" method="POST" id="repasForm">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    ...
                    <input type="text" name="nom" id="nom" value="{{ old('nom', $repas->nom) }}" 
                ...
                <div>
                    ...
                    <select name="type_repas" id="type_repas" 
                        ...
                        <option value="petit-dejeuner" {{ old('type_repas', $repas->type_repas) == 'petit-dejeuner' ? 'selected' : '' }}>🌅 Petit-déjeuner</option>
                        <option value="dejeuner" {{ old('type_repas', $repas->type_repas) == 'dejeuner' ? 'selected' : '' }}>☀️ Déjeuner</option>
                        <option value="diner" {{ old('type_repas', $repas->type_repas) == 'diner' ? 'selected' : '' }}>🌙 Dîner</option>
                        <option value="collation" {{ old('type_repas', $repas->type_repas) == 'collation' ? 'selected' : '' }}>🍴 Collation</option>
                    </select>
                </div>
                        ...
                        <div class="ingredient-item bg-gray-50 p-4 rounded-lg mb-4">
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                                <div>
                                    ...
                                    <select name="ingredients[{{ $index }}][id]" class="ingredient-select mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
                            ...
                            <div class="mt-2 p-2 bg-blue-50 rounded text-sm ingredient-preview">
                                <span class="calories-preview"></span>
                            </div>
                        </div>
                    @endforeach
                </div>
            ...
            <div class="mb-6 p-4 bg-blue-50 rounded-lg">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div class="text-center">
                        <div class="text-2xl font-bold text-blue-600" id="total-calories">{{ round($repas->calories_total) }}</div>
                        <div class="text-sm text-gray-600">Calories</div>
                    </div>
                    <div class="text-center">
                        <div class="text-2xl font-bold text-red-600" id="total-proteines">{{ round($repas->proteines_total, 1) }}</div>
                        <div class="text-sm text-gray-600">Protéines (g)</div>
                    </div>
                    <div class="text-center">
                        <div class="text-2xl font-bold text-yellow-600" id="total-glucides">{{ round($repas->glucides_total, 1) }}</div>
                        <div class="text-sm text-gray-600">Glucides (g)</div>
                    </div>
                    <div class="text-center">
                        <div class="text-2xl font-bold text-purple-600" id="total-lipides">{{ round($repas->lipides_total, 1) }}</div>
        ...
        const template = `
            <div class="ingredient-item bg-gray-50 p-4 rounded-lg mb-4">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Ingrédient *</label>
                        <select name="ingredients[${ingredientIndex}][id]" class="ingredient-select mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
                            <option value="">Choisir un ingrédient</option>
